...
export const QuickNotes: React.FC<QuickNotesProps> = ({
  ...
  const categories: { key: NoteItem['category']; label: string; color: string }[] = [
    { key: 'deneme', label: 'Deneme', color: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20' },
    { key: 'fikir', label: 'Fikir', color: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20' },
    { key: 'todo', label: 'Görev', color: 'bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border-cyan-500/20' },
    { key: 'not', label: 'Not', color: 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20' },
  ];
    ...
    <div className="bg-white dark:bg-neutral-900 rounded-3xl p-6 sm:p-7 border border-zinc-200 dark:border-neutral-800 shadow-sm flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs font-bold text-zinc-500 dark:text-neutral-500 uppercase tracking-widest flex items-center gap-1.5">
          ...
          Not & Scratchpad Bento ({notes.length})
        </span>
      ...
      <form onSubmit={handleSubmit} className="mb-4 space-y-2.5">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Bir deneme notu veya fikir yazın..."
          ...
          <button
            type="submit"
            className="px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-neutral-950 rounded-2xl text-xs font-bold flex items-center gap-1 transition-all active:scale-95 shadow-sm"
          >
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
          <span className="text-[11px] text-zinc-400 dark:text-neutral-500 mr-1 flex items-center gap-1 font-mono">
            <Tag className="w-3 h-3 text-emerald-500" /> Etiket:
          </span>
          {categories.map((c) => (
            <button
              type="button"
        ...
        <AnimatePresence mode="popLayout">
          {filteredNotes.length === 0 ? (
            ...
            >
              ...
              <span>Henüz kaydedilmiş not bulunmuyor.</span>
              <span className="text-[11px] text-zinc-500">Yukarıdaki formdan yeni bir deneme ekleyin.</span>
            </motion.div>
          ) : (
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="text-zinc-400 hover:text-emerald-500 transition-colors flex-shrink-0"
                  >
                    {item.completed ? (
                  ...
                  <button
                    ...
                    className="p-1.5 text-zinc-400 hover:text-rose-500 transition-colors rounded-lg hover:bg-zinc-100 dark:hover:bg-neutral-800"
                    title="Sil"
                  >
